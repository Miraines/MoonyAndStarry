...
type Config struct {
	...
	CookieDomain      string `json:"cookie_domain" env:"COOKIE_DOMAIN"`
}
...
func Load() (*Config, error) {
	...
	if cfg.HTTPSCertFile == "" {
		return nil, fmt.Errorf("HTTPS_CERT_FILE не задан")
	}
	if cfg.HTTPSKeyFile == "" {
		return nil, fmt.Errorf("HTTPS_KEY_FILE не задан")
	}

	if cfg.Issuer == "" {
		return nil, fmt.Errorf("JWT_ISSUER не задан")
	}
	if cfg.Audience == "" {
		return nil, fmt.Errorf("JWT_AUDIENCE не задан")
	}
